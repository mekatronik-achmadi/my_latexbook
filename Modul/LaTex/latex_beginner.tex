...
\usepackage{listings} % paket lingkungan kode sumber alternatif
\usepackage{pdfpages} % paket untuk menambahkan PDF sebagai page
%\usepackage{parskip} % paket untuk tambahan spasi antar paragraf
...
%% this custom command found problematic even in GNU/Linux
%\newcommand{\ShowOsVersion}{
%	\immediate\write18{\unexpanded{foo=`uname -sro` && echo "${foo}" > tmp.tex}}
%	\input{tmp}\immediate\write18{rm tmp.tex}
%}

%% this custom command will not work if not run GNU/Linux
...
%%%%% Modifikasi kotak untuk lstlisting %%%%%
\lstset{
...
\begin{document}
    ...
    \begin{titlepage}
		...
		\raggedright
		\noindent Buku ini ditulis:\\ % tanda \\ menambahkan garis baru
		%OS : \ShowOsVersion \\
		%TeX : \ShowTexVersion \\
...
% lingkungan untuk gambar
	...
	Jika belum, dapat ditambahkan opsi tersebut pada menu \menu[,]{Options,Configure TexStudio}.
	Kemudian pilih tab \menu{Commands}.
...
% lingkungan untuk gambar
	...
	Kemudian ekstensi LaTeX-Workshop dapat diinstal di program VSCodium sendiri pada menu \menu[,]{View,Extension}.
...
$> make clean
	...
	\begin{enumerate}
		\item Klik kanan berkas ISO yang telah didownload, klik \menu{Mount}.
		...
		\item Buka \menu{CMD} di start menu Windows, kemudian ketik dua perintah berikut:
		\begin{minted}[frame=lines,framesep=2mm,fontsize=\normalsize,bgcolor=LightGray]{tex}
> E:
> install-tl-windows.bat --location "E:/"
		...
		Kemudian klik tombol \menu{Advanced}.
		...
		\item Setelah tampilan \menu{Advanced} muncul, perhatikan sisi \menu{Options}.

		Pengaturan yang perlu dilakukan disini:
		\begin{enumerate}
			\item \menu{Adjust searchpath}. Pastikan telah dicentang.
			\item \menu{File associations}. Pilih \menu{All}.
			\item \menu{Install TeXworks front end}. Pastikan TIDAK dicentang (karena disini akan menggunakan TexStudio).
		\end{enumerate}
		...
		\newpage
		\item Perhatikan sisi \menu{Selections}. Klik \menu{Customize}.
		...
		Klik \menu{None}, kemudian centang hanya \menu{US and UK English}.
		...
		\item Klik \menu{Apply} atau \menu{OK}.
		...
		\item Selanjutnya, klik \menu{Install}.
	...
	Kemudian instal sebagai program lainnya, ikuti tombol \menu{Next} dan cukup gunakan pilihan-pilihan default.
...
\end{document}
	...
	\begin{itemize}
		\item Klik menu \menu[,]{Tools, Build and View}.
	...
	\begin{itemize}
		\item Klik menu \menu[,]{Terminal, New Terminal} (jika belum ada terminal tab terbuka).
...
@article{doejohn2500,
	...
	Kemudian berikut contoh dokumen dengan sitasi dengan tag \textbf{cite} dan bibtex di atas (disimpan sebagai \textbf{cobabib.tex}):
...
$> pdflatex -shell-escape -synctex=1 -interaction=nonstopmode cobabib.tex
	...
	\textbf{TIPS:} Untuk kompilasi di command-line, jalankan kompilasi bibtex setiap kali menambah sitasi baru dengan tag \textbf{cite}.
...
\addto\captionsenglish{\renewcommand{\refname}{Daftar Pustaka}} % untuk article
	...
	\begin{enumerate}
		\item Klik menu \menu[,]{Tools,Options}.
		...
		\end{itemize}
		\item Untuk pilihan scope, ambil \menu{for my whole library}.
...
\usepackage{minted}
	...
	\textbf{Perhatian:} Paket \textbf{minted} menggunakan modul Python Pygmentize dalam prosesnya,
	sehingga tidak disarankan untuk sistem operasi non-Unix seperti Windows dikarenakan kompleksitas instalasi.
...
\end{minted}
	...
	\textbf{Perhatian:} Disini penulisan kode menggunakan \textbf{lstlisting} agar dikompile dalam style berbeda
	untuk menghindari kerancuan akibat lingkungan bersarang.
...
print(r)
	...
	\textbf{TIPS:} Jika mendapati \textbf{FancyVerbError}, hapus spasi atau tab yang ada di setelah tag \textbf{begin} atau \textbf{end} pada lingkungan \textbf{minted}.

	\newpage
	\subsection{Lstlisting}

	Mengingat paket \textbf{minted} membutuhkan modul Python Pygmentize, terdapat alternatif lain yang tidak butuh modul lain, yaitu paket \textbf{lstlisting}.
	Hasilnya tidak seperti \textbf{minted}, namun lebih mudah instalasi karena tidak butuh modul Python apa pun.
	Tambahkan paket \textbf{lstlisting} ke dokumen.

	\begin{minted}[frame=lines,framesep=2mm,fontsize=\normalsize,bgcolor=LightGray]{tex}
\usepackage{listings}
	\end{minted}

	Untuk menambahkan kotak latar dan model penulisan, dapat menambahkan pengaturan berikut sebelum \textbf{document}:
...
\lstset{
	...
	Berikut contoh sebelumnya ditulis dalam lingkungan \textbf{lstlisting}.
...
print(r)
	...
	\newpage
	\chapter{Journal of the Acoustic Society of America (JASA)}

	Untuk lebih memperdalam pemahaman, berikut panduan ringkas menggunakan template \LaTeX{} yang tersedia untuk suatu publikasi.
	Untuk bab ini, spesifik publisher Journal of the Acoustic Society of America (JASA) yang digunakan sebagai contoh.
	Berikut langkah-langkah mennggunakan template artikel \LaTeX{} untuk publikasi Journal of the Acoustic Society of America

	\section{Download Template}

	Template artikel yang akan dibahas disini adalah template artikel reprint.
	Untuk mendownload paket template, kunjungi:\\
	\url{https://acousticalsociety.org/preparing-latex-manuscripts/}\\

	Download paket \textit{JASA LaTeX Package} (bukan yang Express Letter atau EL).:\\
	\url{https://acousticalsociety.org/wp-content/uploads/2018/02/2021-05-v-JASA_LaTeXPackage_2.zip}\\

	\textbf{TIPS:} Jika anda lebih memilihi online editor, Overleaf menyediakan template untuk artikel reprint JASA
	\href{https://www.overleaf.com/latex/templates/manuscript-submission-template-for-the-journal-of-the-acoustic-society-of-america/zhqthzfkhhzd}{disini}.

	\section{Memilah/Mengumpulkan}

	Ekstrak berkas zip hasil download ke suatu folder sementara.
	Dapat terlihat tersedia bermacam template dokumen, berkas pendukung, berkas pengaturan, dan berkas PDF contoh kompilasi.
	Untuk dapat memilih dokumen yang dibutuhkan, perhatikan penjelasan ringkas pada berkas \textbf{readme.txt},
	atau berkas \textbf{JASA-TeXGuide.pdf} untuk penjelasan lebih detil.

	\begin{figure}[!ht]
		\centering
		\includegraphics[width=250pt]{images/jasa_files}
		\caption{Berkas Paket Template JASA}
	\end{figure}

	Selanjutnya, buat folder utama untuk proses penulisan.
	Dengan mengikuti panduan di \textbf{readme.txt}, salin berkas-berkas berikut dari folder sementara ekstraksi ke folder utama penulisan:
	\begin{enumerate}
		\item \textbf{reprintsample.tex}. Disini adalah berkas acuan untuk memulai menulis.

		\item \textbf{JASA.cls}. Berkas cls adalah berkas untuk tag \textbf{documentclass} yang dibuat oleh JASA sendiri.

		\bigskip

		\textbf{TIPS:} Umumnya setiap publisher sudah membuat berkas class atau cls sendiri-sendiri, tidak menggunakan bawaan seperti articles atau report.
		Pastikan berkas-berkas cls ini ikut disalin ke folder utama penulisan.

		\item \textbf{jasanum2.bst} dan \textbf{jasaauthyear2.bst}. Berkas bibliography style (bst) yang digunakan di dalam berkas JASA.cls.

		\item \textbf{sampbib.bib}. Berkas contoh database bibliography. Nanti dapat diganti dengan berkas database bibliography anda sendiri.

		\item \textbf{figsampe.jpg}. Berkas contoh gambar. Nanti dapat diganti dengan berkas gambar anda sendiri.
	\end{enumerate}

	\begin{figure}[!ht]
		\centering
		\includegraphics[width=250pt]{images/jasa_files_reprint}
		\caption{Berkas template artikel reprint untuk JASA}
	\end{figure}

	\section{Tes Kompilasi}

	Sebelum memulai menulis, perlu dilakukan tes kompilasi template tersebut ke PDF untuk memastikan:
	\begin{itemize}
		\item Instalasi sistem \LaTeX{} sudah memenuhi minimum kebutuhan.
		\item Berkas-berkas minimum telah lengkap.
	\end{itemize}

	Kompilasi dengan TexStudio editor sebagaimana telah dilakukan sebelumnya.
	Jika menggunakan command-line, berikut contoh perintahnya:

	\begin{minted}[frame=lines,framesep=2mm,fontsize=\normalsize,bgcolor=LightGray]{sh}
$> pdflatex -shell-escape -synctex=1 -interaction=nonstopmode reprintsample.tex
$> bibtex reprintsample.aux
$> pdflatex -shell-escape -synctex=1 -interaction=nonstopmode reprintsample.tex
$> pdflatex -shell-escape -synctex=1 -interaction=nonstopmode reprintsample.tex
	\end{minted}

	Kemudian jika sukses menghasilkan berkas PDF, bandingkan antara berkas PDF tersebut, dengan berkas PDF dari ekstraksi di folder sementara sebelumnya.
	Jika sepenuhnya sama, maka instalasi sistem \LaTeX{} maupun berkas-berkas di folder utama penulisan siap digunakan.

	\section{Memulai menulis}

	Untuk memulai menulis menggunakan template, lebih disarakan untuk membuat berkas dokumen baru, kemudian lakukan copy-paste setiap tag dan lingkungan
	dari berkas dokumen template ke dokumen yang dikerjakan, ketimbang melakukan editing ke dokumen template langsung.
	Dengan demikian dokumen template dapat digunakan sebagai contoh atau acuan.

	Beberapa tag/lingkungan yang penting untuk di copy-paste terlebih dahulu:

	\begin{itemize}
		\item tag \textbf{documentclass}. Pastikan menggunakan kelas JASA.

		\item tag \textbf{title}.

		\item tag \textbf{author} dan \textbf{affiliation}.

		\item lingkungan \textbf{abstract}.

		\item tag \textbf{maketitle}.

		\item \textbf{section} dan \textbf{subsection} sesuai kebutuhan.

		\item tag \textbf{bibliography}. Sesuaikan dengan berkas bibtex yang anda pakai.
	\end{itemize}

	\subsection{Kerangka Dokumen}

	Berikut kerangka dokumen dasar berdasarkan berkas template \textbf{reprintsample.tex}.
	Dokumen baru ini dapat disimpan sebagai \textbf{paperorca.tex}.

	\begin{minted}[frame=lines,framesep=2mm,fontsize=\normalsize,bgcolor=LightGray]{tex}
\documentclass[reprint]{JASA}

\begin{document}
	\title[JASA/Sample Article]{Article title should be less than 17 words, no acronyms}
...
\end{document}
	...
	Berikut contoh konten judul yang akan ditampilkan oleh tag \textbf{maketitle}
...
\affiliation{Teknik Perkulian,  Universitas Kehidupan, Bogor, 11111, Indonesia}
	...
	Contoh abstrak yang dapat ditulis dalam lingkungan abstract
...
\end{abstract}
	...
	Sesuaikan berkas bibtex sesuai yang anda gunakan.

	Contoh berkas bibtex untuk topik relevan dengan judul (simpan dengan nama \textbf{orca.bib}) dapat didownload atau copy-paste
	\href{https://github.com/mekatronik-achmadi/my_latexbook/blob/master/Modul/LaTex/files/jasa/orca.bib}{disini}.

	Edit tag \textbf{bibliography} seperti berikut:

	\begin{minted}[frame=lines,framesep=2mm,fontsize=\normalsize,bgcolor=LightGray]{tex}
\bibliography{orcabib}
	\end{minted}

	kemudian lakukan tes kompilasi dan bibtex seperti sebelumnya melalui TexStudio atau perintah command-line:

	\begin{minted}[frame=lines,framesep=2mm,fontsize=\normalsize,bgcolor=LightGray]{sh}
$> pdflatex -shell-escape -synctex=1 -interaction=nonstopmode paperorca.tex
$> bibtex paperorca.aux
$> pdflatex -shell-escape -synctex=1 -interaction=nonstopmode paperorca.tex
$> pdflatex -shell-escape -synctex=1 -interaction=nonstopmode paperorca.tex
	\end{minted}

	\subsection{Konten Selanjutnya}

	Setelah bibliografi dan tes kompilasi, penulisan konten selanjutnya dapat diteruskan.
	Namun tetap memperhatikan petunjuk (seperti pada PDF \textbf{JASA-TeXGuide.pdf})
	maupun komentar-komentar penjelasan di template.

	\bigskip

	Berikut contoh konten introduksi dengan sitasi.
...
Pola tersebut spesifik terhadap setiap grup keluarga yang bersifat matrinial \cite{Miller2004}.
	...
	\begin{center}
		\textbf{** Silahkan dilanjutkan sendiri sesuai kebutuhan **}
	\end{center}

	\newpage
	\subsection{Contoh Menyeluruh}

	Berikut contoh konten secara menyeluruh hingga langkah ini:
...
\begin{document}
	...
	\begin{abstract}
		Mamalia dalam ordo \textit{Cetacea} dikenal sebagai
		mamalia \textit{pelagic} yang bersifat sosial.
		Dalam setiap komunitasnya, ditemukan perilaku nyanyian yang dapat dideteksi
		dalam air hingga puluhan kilometer.
		Dengan analisa spektrum audio, dapat ditemukan pola yang konsisten
		dan berulang untuk setiap grup keluarga.
...
\end{document}
	...
	Hasil PDF pada halaman selanjutnya

	\newpage
	%\includepdf[pages=-]{files/jasa/paperorca.pdf} % Contoh paper hasil contoh untuk semua halaman
	\includepdf[pages={1}]{files/jasa/paperorca.pdf} % Contoh paper hasil contoh untuk halaman pertama
